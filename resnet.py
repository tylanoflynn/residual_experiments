...
class BlockDrop(Module):
    ...
    def forward(self, x):
        if self.training:
            x = Variable(torch.bernoulli(1 - self.prob)) * x
            return x
        else:
            return x
    ...
    def blockBasic(nInputPlane, nOutputPlane, stride):
        ...
        for i,v in enumerate(convParams):
            if i == 0:
                ...
                convs.add_module('Conv' + str(i), nn.Conv2d(nBottleneckPlane, nBottleneckPlane, *v))

        shortcut = (nInputPlane == nOutputPlane) and Identity() or nn.Conv2d(nInputPlane, nOutputPlane, 1, stride, 0)
